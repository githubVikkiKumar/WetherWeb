...
import { GoogleMapsService, LocationData } from './google-maps.service';
...
export class WeatherService {
  ...
  constructor(
    ...
    this.setupGoogleMapsLocationTracking();
  }
  ...
  // Setup Google Maps location tracking
  private setupGoogleMapsLocationTracking(): void {
    // Wait for Google Maps to be ready
    const checkGoogleMaps = setInterval(() => {
      if (this.googleMapsService.isGoogleMapsReady()) {
        clearInterval(checkGoogleMaps);
        this.startGoogleMapsLocationTracking();
      }
    }, 100);

    // No fallback - only use Google Maps
    setTimeout(() => {
      if (!this.googleMapsService.isGoogleMapsReady()) {
        clearInterval(checkGoogleMaps);
  ...
  // Start Google Maps location tracking
  private startGoogleMapsLocationTracking(): void {
    ...
    this.googleMapsService.getCurrentLocation()
      .then((locationData: LocationData) => {
        ...
        this.cityName = `${locationData.city}, ${locationData.country}`;
    ...
    this.googleMapsService.startLocationTracking();

    // Subscribe to location updates
    this.googleMapsService.getLocationUpdates().subscribe((locationData: LocationData | null) => {
      if (locationData) {
        const newLocation = {
          latitude: locationData.latitude,
          longitude: locationData.longitude
        ...
        if (this.userLocation && this.googleMapsService.calculateDistance(
          this.userLocation.latitude, this.userLocation.longitude,
          newLocation.latitude, newLocation.longitude
        ) > 0.5) { // 500 meters
          this.userLocation = newLocation;
          this.cityName = `${locationData.city}, ${locationData.country}`;
          this.getData(); // Refresh weather data
        }
      }
    });
  }

  // Show location error when Google Maps fails
  private showLocationError(): void {
    this.cityName = 'Location not available';
    ...
    this.temperatureData = {
      ...
      rainPercent: 0,
      summaryPhrase: 'Location access required',
  ...
  getData(){
    ...
    // Only use Google Maps coordinates for weather data
    if (this.userLocation && this.isLocationDetected) {
      this.getWeatherByCoordinates(this.userLocation.latitude, this.userLocation.longitude);
    } else {
      this.showLocationError();
    }
  }
  ...
  // Get weather data by coordinates for more accurate results
  private getWeatherByCoordinates(lat: number, lon: number): void {
    const apiKey = EnvironmentalVariables.openWeatherMapApiKey;
    const weatherUrl = `${EnvironmentalVariables.openWeatherMapBaseURL}weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`;
    
    this.httpClient.get(weatherUrl).subscribe({
      next: (response: any) => {
        this.processOpenWeatherMapData(response);
        this.isLoading = false;
      },
      error: (error) => {
        // If OpenWeatherMap fails, show error
  ...
  // Method to get current location name
  getCurrentLocationName(): string {
    return this.cityName;
  }

  // Method to check if location is being tracked
  ...
  // Cleanup method to stop location tracking
  ngOnDestroy(): void {
    ...
    if (this.weatherErrorSubscription) {
      this.weatherErrorSubscription.unsubscribe();
    }
    // Stop Google Maps location tracking
    this.googleMapsService.stopLocationTracking();
  }
